...
time_series_data <- data.frame(
  Model = c("ARIMA", "ETS"),
  RMSE = c(5.21153, 5.35532)
)

# Data for Regression Models
regression_data <- data.frame(
  Model = c("Random Forest", "XGBoost", "KNN", "Linear Regression"),
  RMSE = c(10.91327, 12.73847, 11.15327, 12.84890)
)

# Donut Chart for Time Series Models
ggplot(time_series_data, aes(x = 2, y = RMSE, fill = Model)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  xlim(0.5, 2.5) +
  scale_fill_manual(values = c("#1f78b4", "#33a02c")) +
  geom_text(aes(label = paste0("RMSE: ", RMSE)), position = position_stack(vjust = 0.5), size = 5) +
  ggtitle("Time Series Model Performance") +
  theme_void() +
  theme(
...
# Donut Chart for Regression Models
ggplot(regression_data, aes(x = 2, y = RMSE, fill = Model)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  xlim(0.5, 2.5) +
  scale_fill_manual(values = c("#6a3d9a", "#ff7f00", "#b15928", "#e31a1c")) +
  geom_text(aes(label = paste0("RMSE: ", RMSE)), position = position_stack(vjust = 0.5), size = 5) +
